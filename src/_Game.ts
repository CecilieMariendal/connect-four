...
export default class Game {
    ...
    private drawMarker(event: MouseEvent): void {
        ...
        if (event.clientX < this.canvas.offsetLeft + 50) {
            positionX = 50;
        } else if (event.clientX > this.canvas.offsetLeft + this.canvas.width - 50) {
        ...
        this.context.fillStyle = this.getPlayerGradient(positionX, 50, (this.getAvailableCell(indexX) !== null) ? this.currentPlayer : 'invalid');
        this.context.fill();
    }
    ...
    private computerTurn(): void {
        for (let x = 0; x < 7; x++) {
            ...
            const blueWouldWin = this.checkIfStreak('blue', 4, {
                y,
                x
            });
            if (blueWouldWin) {
                this.grid[y][x] = 'blue';
                this.previousIndex.push({
                    y,
                    x
                });
                this.lastIndex = {
            ...
            const redWouldWin = this.checkIfStreak('red', 3, {
                y,
                x
            });
            if (redWouldWin) {
